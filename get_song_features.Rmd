...
```{r}
users <- read.csv('user_id.csv', header = TRUE, sep = ';')
```
...
```{r}
  # get song features from playlists
for (i in all_playlists$id)   {
...
all_data <- merge(x = all_playlists, y = test, by.x = "id", by.y = "playlist_id", all.x = TRUE)
```
